...
export function TestScreen({ 
  ...
  return (
    <div data-testid="test-screen" className="overflow-hidden max-w-full w-full">
      {/* Progress bar */}
      <div className="mb-8 px-4 max-w-full overflow-hidden">
        <div className="flex justify-between items-center mb-2 max-w-full">
          <span className="text-sm font-medium text-slate-600 truncate">{t('test.progressLabel', '진행률')}</span>
          <span className="text-sm font-medium text-primary truncate" data-testid="text-progress">
            {t('test.progress', { current: currentQuestionIndex + 1, total: totalQuestions })}
          </span>
        </div>
        <Progress value={progress} className="h-2 max-w-full" data-testid="progress-bar" />
      </div>

      {/* Question card */}
      <Card className="rounded-2xl shadow-lg max-w-full overflow-hidden">
        <CardContent className="p-6 md:p-8 lg:p-12 max-w-full overflow-hidden">
          <div className="text-center mb-6 md:mb-8 max-w-full overflow-hidden">
          ...
          <div className="space-y-4 max-w-2xl mx-auto px-4 w-full overflow-hidden">
            <Button
