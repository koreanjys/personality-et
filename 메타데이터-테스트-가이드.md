...
```
언어: "ko-KR" (또는 "ko")
제목: "무료 성격유형 테스트 | 성격유형 & 에겐테토 궁합 테스트"
설명: "무료 성격유형 테스트로 당신의 성격유형을 알아보고 에겐-테토와의 궁합을 확인해보세요! 16가지 성격 유형별 상세 분석과 캐릭터 궁합도를 제공합니다."
OG언어: "ko_KR"
```
...
```
언어: "en-US" (또는 "en")
제목: "Free Personality Type Test | Personality Type & Egen-Teto Compatibility Test"
설명: "Discover your personality type and check your compatibility with Egen-Teto characters! Get detailed analysis of 16 personality types and character compatibility ratings."
OG언어: "en_US"
```
...
**예상 결과:**
- 제목: `"ENFJ + 에겐 성격 유형 | 무료 성격유형 테스트 결과"`
...
**예상 결과:**
- 제목: `"ENFJ + Egen Personality Type | Free Personality Type Test Results"`
...
### 한국어 브라우저 (`ko-KR`)
- [ ] 홈페이지 제목: "무료 성격유형 테스트 | 성격유형 & 에겐테토 궁합 테스트"
- [ ] 홈페이지 설명에 "무료 성격유형 테스트로 당신의 성격유형을 알아보고 에겐-테토와의 궁합을 확인해보세요!" 포함  
- [ ] `og:locale` = `"ko_KR"`
- [ ] 결과 페이지 제목: "성격 유형 | 무료 성격유형 테스트 결과" 형식 (예: "ENFJ + 에겐 성격 유형 | 무료 성격유형 테스트 결과")
- [ ] 결과 페이지에서 "에겐/테토" 한국어 표시
- [ ] 소셜 미디어 미리보기에서 한국어 제목/설명

### 영어 브라우저 (`en-US`)
- [ ] 홈페이지 제목: "Free Personality Type Test | Personality Type & Egen-Teto Compatibility Test"
- [ ] 홈페이지 설명에 "Discover your personality type and check your compatibility with Egen-Teto characters!" 포함
- [ ] `og:locale` = `"en_US"`  
- [ ] 결과 페이지 제목: "Personality Type | Free Personality Type Test Results" 형식 (예: "ENFJ + Egen Personality Type | Free Personality Type Test Results")
...
function checkMetadata() {
  ...
  if (actualIsKorean) {
    const expectedTitle = '무료 성격유형 테스트 | 성격유형 & 에겐테토 궁합 테스트';
    const expectedDesc = '무료 성격유형 테스트로 당신의 성격유형을 알아보고 에겐-테토와의 궁합을 확인해보세요!';
    const expectedLocale = 'ko_KR';
    
    console.log(data.title === expectedTitle ? '✅ 한국어 제목 정상' : '❌ 한국어 제목 오류');
    console.log(data.description && data.description.includes('무료 성격유형 테스트로 당신의 성격유형을') ? '✅ 한국어 설명 정상' : '❌ 한국어 설명 오류');
    console.log(data.ogLocale === expectedLocale ? '✅ 한국어 Locale 정상' : '❌ 한국어 Locale 오류');
  } else {
    const expectedTitle = 'Free Personality Type Test | Personality Type & Egen-Teto Compatibility Test';
    const expectedDesc = 'Discover your personality type and check your compatibility with Egen-Teto characters!';
    const expectedLocale = 'en_US';
    
    console.log(data.title === expectedTitle ? '✅ 영어 제목 정상' : '❌ 영어 제목 오류');
    console.log(data.description && data.description.includes('Discover your personality type and check your compatibility') ? '✅ 영어 설명 정상' : '❌ 영어 설명 오류');
